...
def demand_function(Y, n_products, P, method="hyperbolic", sigma=1, P0=1, alpha=None, beta=1, theta=None):
    """
    ...
    N = n_products # Number of products
    D = np.zeros(N)  # Initialize demand array

    # ** Original Hyperbolic Demand**
    if method == "hyperbolic":
        D = Y / (N * P)

    # ** CES (Constant Elasticity of Substitution) Demand**
    elif method == "ces":
        price_weighted = P ** (-sigma)  # Compute price elastic effect
        D = Y * price_weighted / np.sum(price_weighted)  # Normalize demand

    # ** Nonlinear (S-Shaped) Demand**
    elif method == "nonlinear":
        price_adjusted = 1 / (1 + np.exp(-(P - P0)))  # S-curve function
        D = (Y / P) * price_adjusted  # Apply the demand saturation effect

    # ** Income-Adjusted Demand (Optional)**
    elif method == "income_sensitive":
        if alpha is None:
            alpha = np.ones(N) / N  # Equal preference if not specified
        D = alpha * (Y ** beta) / P

    # ** Preference-Based Demand (Optional)**
    elif method == "preference":
        if theta is None:
            theta = np.zeros(N)  # No bias by default
        D = (Y / P) * (1 + theta)
