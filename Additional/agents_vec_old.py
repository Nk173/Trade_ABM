    ...
    for t in tqdm(range(iterations)):
        ...
        for nation in range(n_countries):
            for industry in range(n_products):
                ...
                # Domestic demand
                domestic_demand = domestic_income[nation, nation] / (n_products * effective_price_domestic)

                # Foreign demand aggregated over all other nations
                foreign_demand = np.sum([
                    foreign_income[otherNation, nation] / (n_products * effective_price_foreign[otherNation])
                    for otherNation in range(n_countries)
                ])
                
#                 if t>=tariff_time-5:
#                     print('Country:', nation, 'sector:',industry, 'domestic demand:',domestic_demand,'\n',
#                           'foreign demand:',foreign_demand, '\n')
...
#     total_exports=np.sum(net_exports_history, axis=2)
    ...
    if plot_simple:
        ...
        for nation in range(n_countries):
            for industry in range(n_products):
                ax0[0].plot(range(iterations), production[nation, industry, :],
                              label='{}-{}'.format(cnames[nation] if cnames else countries[nation], 
                                                   pnames[industry] if pnames else products[industry]))
                ax0[0].set_title('Production')
                
            ax0[1].plot(range(iterations), (gdp_vec[nation, :] / citizens_per_nation[nation]),
                          label='{}'.format(cnames[nation] if cnames else countries[nation]))
            ax0[1].set_title('GDP per capita', )
#             ax0[1].set_yscale('log')
#             ax0[0].set_yscale('log')
        if t>T0: 
            ax0[0].axvline(x=T0, ls= '--', color='k')
            ax0[1].axvline(x=T0, ls= '--', color='k')
        if t>tariff_time:
            ax0[0].axvline(x=tariff_time, ls= '--', color='k')
            ax0[1].axvline(x=tariff_time, ls= '--', color='k')
            
        # Add labels to ax2 plots
        ax0[0].text(0.95, 0.85, '(a)', transform=ax0[0].transAxes, fontsize=12, va='top', ha='right', weight='bold')
        ax0[1].text(0.95, 0.85, '(b)', transform=ax0[1].transAxes, fontsize=12, va='top', ha='right', weight='bold')

        # Adjust layout to place fig2 below fig1
        fig0.tight_layout()

        handles0, labels0 = ax0[0].get_legend_handles_labels()
        fig0.legend(handles0, labels0, loc='lower left', bbox_to_anchor=(0, -0.04), ncol=n_countries, fontsize=8)
        
        handles00, labels00 = ax0[1].get_legend_handles_labels()
        fig0.legend(handles00, labels00, loc='lower right', bbox_to_anchor=(1, -0.04), ncol=n_countries, fontsize=8)
        
#         ax2[0].set_yscale('log')
        # Save the figures
        fig0.savefig(f'{case}_simple.png')
...
#                     xytext=(cm_time, ax2[1].get_ylim()[0] - (ax2[1].get_ylim()[1] - ax2[1].get_ylim()[0]) * 0.05), rotation=90, va='center', ha='center', fontsize=10, color='black')
        ...
        # Add labels to ax2 plots
        ax2[0].text(0.95, 0.85, '(i)', transform=ax2[0].transAxes, fontsize=12, va='top', ha='right', weight='bold')
        ax2[1].text(0.95, 0.85, '(j)', transform=ax2[1].transAxes, fontsize=12, va='top', ha='right', weight='bold')
    ...
    return Q, income, net_exports, io_df, io_df2, io_dfval, io_df2val, trades_history
